...
local CONFIG = {
    ...
    KEY_STORAGE_FILE = "UltraHackKey.txt" -- Файл для сохранения ключа
}
...
local function saveKey(key)
    pcall(function()
        writefile(CONFIG.KEY_STORAGE_FILE, key)
    end)
end
...
local function loadSavedKey()
    local success, result = pcall(function()
        if isfile(CONFIG.KEY_STORAGE_FILE) then
            return readfile(CONFIG.KEY_STORAGE_FILE)
        end
        return nil
    end)
...
-- Функция запуска второго скрипта (меню чита)
